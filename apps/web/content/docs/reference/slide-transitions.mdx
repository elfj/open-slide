...
type SlideTransition = {
  ...
  /** Matches <SharedElement id="…"> pairs across the outgoing and incoming pages. */
...
pages with `SharedElement` and give both instances the same `id`. The transition
...
content changes, keep that changing copy outside `SharedElement` and let it fade
as normal page content. The runtime does not guess similar objects; the `id` is
the contract.
