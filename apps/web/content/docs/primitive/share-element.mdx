...
`unstable_sharedElement` accepts one child. If the child is a valid React
element, the runtime writes the `data-osd-shared-element` marker onto that
child. Otherwise it wraps the content in a `div`.
